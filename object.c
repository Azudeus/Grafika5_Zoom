...
void zoomObject(Object *O,float mul) {
	for(int i = 0;i < O->size-1 ;i++) {
		O->P[i].x = (int) O->P[i].x * mul;
		O->P[i].y = (int) O->P[i].y * mul;
	}
	for(int i = 0;i < O->nlingkaran ;i++) {
        O->L[i].x = (int)O->L[i].x * mul;
        O->L[i].y = (int)O->L[i].y * mul;
        O->L[i].r = (int)O->L[i].r * mul;
    }
}

void gambarObject(Object O, Matrix* M, char c, float mul) {
	Point start, finish;
	zoomObject(&O,mul);
